resource "aws_sqs_queue" "sale-count-update-queue" {
  ...
  redrive_policy             = jsonencode({
    deadLetterTargetArn = aws_sqs_queue.dead_letter_queue13.arn
    maxReceiveCount     = 5
  })
}

resource "aws_sqs_queue" "dead_letter_queue13" {
  name = "sale-count-update-dead-letter-queue"
}
resource "aws_sqs_queue_policy" "example_queue_policy13" {
