resource "aws_sqs_queue" "product-review-data-update-queue" {
  ...
  redrive_policy            = jsonencode({
    deadLetterTargetArn = aws_sqs_queue.dead_letter_queue11.arn
    maxReceiveCount     = 5
  })
}

resource "aws_sqs_queue" "dead_letter_queue11" {
  name = "product-review-data-update-dead-letter-queue"
}
resource "aws_sqs_queue_policy" "example_queue_policy11" {
